...
class IdentificationTest(unittest.TestCase):
    ...
    def test_analysis(self):
        # given
        data_samples = Path('resources/scans/Niedersach_examples_10/').as_posix()
        reference = Path('resources/scans/Niedersach_examples_10/Sample_Niedersachsen-2-1.png').as_posix()
        output_path = Path('resources/results/7/')
    ...
    def test_no_table_extraction(self):
        reference = Path('../../resources/form1/mask.png').as_posix()
        output_path = Path('resources/results/9/')
        output_path.mkdir(parents=True, exist_ok=True)
